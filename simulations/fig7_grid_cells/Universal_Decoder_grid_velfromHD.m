...
%    peakrate = .005+(ps-1)*.0025;
    ...
    clearvars -except spd tpos hd_real ti vscale tauInh ps pmod centerfreq cf cf_* ncells basefrequency peakrate tau cosTuningCurve* seeds t1 t2 dt sigma networkType test* train* Count_*;
...
%     dfh = designfilt('bandpassiir', 'StopbandFrequency1', .01, 'PassbandFrequency1', .02, 'PassbandFrequency2', .2*1.8, 'StopbandFrequency2', .2*1.9, 'StopbandAttenuation1', 30, 'PassbandRipple', 1, 'StopbandAttenuation2', 30, 'SampleRate', 30);
    %rav=[0; abs(circ_dist(hd_real(2:end),hd_real(1:end-1)))]-.01;
    %rav(find(rav<0))=0;
    %bpav=rav;
    %bpav=bpav-.25*min(bpav);
    %bpav(find(bpav<.25*max(bpav)))=0;
 %   gw=90;
 %   rav=6*conv(rav,gausswin(gw)/sum(gausswin(gw)),'same');
...
%    spd=rav(601:end);
%    tpos=tpos(601:end); tpos=tpos-tpos(1);
%    xpos=hd_real(601:end);
    figure(1); clf;
    plot(tpos(1:30*30),spd(1:30*30));
    figure(2); clf;
    plot(tpos(1:30*30),xpos(1:30*30));
    figure(3); clf; histogram(50*spd/(2*pi),0:2:100);
    drawnow;
    tic;
