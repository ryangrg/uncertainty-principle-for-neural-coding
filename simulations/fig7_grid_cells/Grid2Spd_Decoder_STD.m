...
%     tpos=tpos-tpos(1);
    ...
    %
    ihd(:,1) = ixpos; %interpolateHDData(hd, tpos, dt, t1, t2);
...
%    AV(325039:(325039+311705))=-AV(325039:-1:trainstart);
    ...
    %seed=1;
    Spikes = spikeMatrix(trainrange,:); %CV0spikeGenerator(ihd(trainrange), itpos(trainrange), ncells, peakrate, sigma); %create spike trains
...
%    withinGstd = Generate_Conductance(itpos(trainrange), ncells, Spikes-Gstd, tau_within); %convolve with exponential decay kernel
    ...
    Spikes = spikeMatrix(testrange,:); %CV0spikeGenerator(ihd(testrange), itpos(testrange), ncells, peakrate, sigma); %create spike trains
