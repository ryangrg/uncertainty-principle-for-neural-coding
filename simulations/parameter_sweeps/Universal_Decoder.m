...
%    peakrate = .005+(6-1)*.0025;
    ...
    clearvars -except ti vscale tauInh ps centerfreq cf cf_* ncells basefrequency peakrate tau cosTuningCurve* seeds t1 t2 dt sigma networkType test* train* Count_*;
...
%    AV(325039:(325039+311705))=-AV(325039:-1:trainstart);
    ...
    [CV0_BW_weights, CV0_BetWith_weights, CV0_trainerr, CV0_trainshift] = Universal_Train(ihd(trainrange), AV(trainrange), CV0_Spikes, CV0_G);
    ...
    [CV0_testerr, CV0_testshift] = Universal_Test(ihd(testrange), AV(testrange), CV0_Spikes, CV0_G, CV0_BW_weights, CV0_BetWith_weights);
...
%    figure(300); subplot(4,4,centerfreq); plot(-31:2:31,CV0_BetWith_weights(:,5)); axis tight;
    ...
    drawnow;
        ...
        [CV1_BW_weights, CV1_BetWith_weights, CV1_trainerr, CV1_trainshift] = Universal_Train(ihd(trainrange), AV(trainrange), CV1_Spikes, CV1_G);
        ...
        [CV1_testerr, CV1_testshift] = Universal_Test(ihd(testrange), AV(testrange), CV1_Spikes, CV1_G, CV1_BW_weights, CV1_BetWith_weights);
...
%         seed=1;
%         CV1_Spikes = vonMisesFiringProb(ihd(trainrange), ncells, .15, sigma, seed); %create spike trains
...
%         [Comp_BW_weights, Comp_BetWith_weights, Comp_trainerr, Comp_trainshift] = Universal_Train_Diff(1, itpos(trainrange), ihd(trainrange), AV(trainrange), Comp_Spikes, Comp_G);
%     
%         CV1_Spikes = vonMisesFiringProb(ihd(testrange), ncells, .15, sigma, seed); %create spike tests
...
%         [Comp_testerr, Comp_testshift] = Universal_Test_Diff(1, itpos(testrange), ihd(testrange), AV(testrange), Comp_Spikes, Comp_G, Comp_BW_weights, Comp_BetWith_weights);
...
%     drawnow;
    ...
    %
    %     ThetaSpikes = PlacePhaseBurstGenerator(ihd(trainrange), itpos(trainrange), ncells, [1:(ncells-1)]/2, basefrequency, peakrate); %create spike trains
    ...
    %
    %     ThetaSpikes = PlacePhaseBurstGenerator(ihd(testrange), itpos(testrange), ncells, [1:(ncells-1)]/2, basefrequency, peakrate); %create spike trains
...
figure(1); clf; 
subplot(1,2,1);
plot(Count_CV1',cf_CV1_testerr(:,3));
hold on; plot(Count_CV0',cf_CV0_testerr(:,3));
hold on; plot(Count_Comp',cf_Comp_testerr(:,3));
subplot(1,2,2);
...
figure(2); clf; 
subplot(1,2,1);
plot(Count_CV1',cf_CV1_testerr(:,5));
hold on; plot(Count_CV0',cf_CV0_testerr(:,5));
hold on; plot(Count_Comp',cf_Comp_testerr(:,5));
subplot(1,2,2);
plot(Count_CV1',cf_CV1_testshift(:,5));
hold on; plot(Count_CV0',cf_CV0_testshift(:,5));
hold on; plot(Count_Comp',cf_Comp_testshift(:,5));
