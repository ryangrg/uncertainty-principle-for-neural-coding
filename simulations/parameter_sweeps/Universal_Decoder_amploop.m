...
pmod=[1.53 0.75 0.65 .715 0.9];
tauInh = [.04 .19 .27 .26 .21];
%for centerfreq=1:15
for ps=1
%for vscale=1:4
...
%    peakrate = .005+(ps-1)*.0025;
    peakrate = (.005+(7-1)*.0025)*pmod(1);
    ti=tauInh(1);
    
    cf=.1;%*centerfreq;
    
    clearvars -except ti vscale tauInh ps pmod centerfreq cf cf_* ncells basefrequency peakrate tau cosTuningCurve* seeds t1 t2 dt sigma networkType test* train* Count_*;
...
%    dfh = designfilt('bandpassiir', 'StopbandFrequency1', cf*.1, 'PassbandFrequency1', cf*.2, 'PassbandFrequency2', cf*1.8, 'StopbandFrequency2', cf*1.9, 'StopbandAttenuation1', 30, 'PassbandRipple', 1, 'StopbandAttenuation2', 30, 'SampleRate', 30);
    ...
    bpav=filtfilt(dfh,av)*.75;%*2*pi/30;
    figure(1); clf;
    subplot(2,1,1);
    ...
    plot(tpos((trunc*30:(duration-trunc)*30)),av((trunc*30:(duration-trunc)*30))); hold on; plot(tpos((trunc*30:(duration-trunc)*30)),360*bpav((trunc*30:(duration-trunc)*30))/(2*pi/30));
    subplot(2,1,2);
