...
%for vscale=1:4
 ...
 %   ps=8;
    sigma=0.5;
    peakrate = .01+(ps-1)*.01;%.005+(ps-1)*.0025;
...
tau_within=.1*2^(centerfreq-1); tau_between=tau_within;
...
%    AV(325039:(325039+311705))=-AV(325039:-1:trainstart);
    ...
    %withinGstd = Generate_STD(itpos(trainrange), ncells, Spikes, .5, .9); %convolve with exponential decay kernel
    %betweenG = Generate_Between_STD(itpos(trainrange), Spikes, withinG, withinGstd, tau_between);
    ...
    %withinGsat = Generate_Saturating_Conductance(itpos(testrange), ncells, Spikes, tau_within); %convolve with exponential decay kernel
...
%     drawnow;
        ...
        withinG = Generate_Conductance(itpos(trainrange), ncells, Spikes, tau_within); %convolve with exponential decay kernel
        %withinGsat = Generate_Saturating_Conductance(itpos(trainrange), ncells, Spikes, tau_within); %convolve with exponential decay kernel
        betweenG = Generate_Between(itpos(trainrange), Spikes, withinG, tau_between);
