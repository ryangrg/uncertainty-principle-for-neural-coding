function [ SpikeMatrix ] = PlacePhaseSpikeGenerator(interHD, itpos, neuronNum, cyclesperlap, basefreq)
...
%     firingRate = 0.001./((firingProbability - repMinFiringProbability)./K);
    ...
    for i = 1:size(itpos, 1)
        ...
        for j=2:neuronNum
            targetInterval = mod((cyclesperlap(j-1)*interHD(i)/(2*pi))*baseinterval,baseinterval);
            timeSinceLastRef = abs(itpos(i) - lastRefSpike);
